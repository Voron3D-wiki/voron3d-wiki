...
// NOTE ON NAVIGATION
//
// The sidebar below regroups the site into four sections, but no file has
// moved: every page still lives at the same path and serves at the same URL as
// it does under MkDocs. Starlight's sidebar is configured independently of
// where content sits on disk, so the whole navigation rework is achievable with
// zero URL changes and zero redirects.
//
// The grouping follows the order the fans page already uses:
//   concept -> criteria -> options -> reviews -> buy
// Each section leads with its explainer ("overview"), products sit underneath.
...
gtag('config','G-7E70MV2KN4',{transport_type:'beacon',url_passthrough:false,linker:{domains:[],decorate_forms:false},allow_google_signals:false,allow_ad_personalization_signals:false});`,
        ...
        // carried over unchanged from the MkDocs build.
        { tag: 'script', attrs: { src: '/js/analytics.js', defer: true } },
      ],

      editLink: { baseUrl: 'https://github.com/Voron3D-wiki/voron3d-wiki/edit/main/docs/' },
      lastUpdated: true,
